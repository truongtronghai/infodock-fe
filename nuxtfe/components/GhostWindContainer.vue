...
export default {
  ...
  mounted() {
    ...
    this.$axios.$get(this.$config.myApiListPosts + 'news/').then((response) => {
      if (response.count) {
        this.posts = response.results.map((item) => {
          return [item.title, item.excerpt, item.title.replaceAll(' ', '-')]
        })
      }
    })
  },
  methods: {
    async reNew() {
      await this.$axios
        .$get(this.$config.myApiListPosts + 'hybrid/')
        .then((res) => {
          if (res.count) {
            // having records
            this.waitingList = res.results.map((i) => {
              return [i.title, i.title.replaceAll(' ', '-')]
            })
          }
        })
      this.numRan = Math.floor(Math.random() * 10)
      setTimeout(this.reNew, 1000)
    },
  },
  fetchOnServer: false, // if this is true, script does not get from backend (I will learn deeper for reason!)
}
</script>
